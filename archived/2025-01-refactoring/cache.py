...
class CacheService:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats_dict["total_entries"] = 0
        
        # Calculate size from actual cache files
        for cache_dir in [self.stage1_dir, self.stage2_dir]:
            for subdir in cache_dir.iterdir():
                if subdir.is_dir():
                    for cache_file in subdir.glob("*.json"):
                        try:
                            stats_dict["total_size"] += cache_file.stat().st_size
                            stats_dict["total_entries"] += 1
                        except Exception:
                            pass
