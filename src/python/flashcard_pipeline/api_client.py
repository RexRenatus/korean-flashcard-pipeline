...
class OpenRouterClient:
    ...
    async def process_stage1(self, vocabulary_item: VocabularyItem) -> Tuple[Stage1Response, ApiUsage]:
        ...
        response = await self._make_request(request.dict())
        ...
        try:
            stage1_data = json.loads(content)
    ...
    async def _make_request(self, payload: Dict[str, Any], retry_count: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            raise NetworkError(f"Network error after {max_retries} retries", original_error=e)
            ...
            raise NetworkError(f"Request timeout after {max_retries} retries", original_error=e)
